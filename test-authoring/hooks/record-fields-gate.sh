...
trap __fc EXIT
# PreToolUse gate (Write|Edit|MultiEdit) — contract v3 s20, test-authoring's own
# required-field set (adapted per issue-167 from roles/test-authoring.json's
...
# docs/issue-<n>/reports/test-authoring.md, require a section per required field
...
TEST_AUTHORING_PAYLOAD="$payload" python3 <<'PY'
import json, os, sys

REQUIRED_FIELDS = ["suite-architecture-note", "fixture-strategy", "smell-list"]
RECORD_SUFFIX = "docs/issue-" # + "<n>/reports/test-authoring.md"

def deny(msg):
    sys.stderr.write("test-authoring: refused — %s\n" % msg)
    sys.exit(2)

payload = os.environ.get("TEST_AUTHORING_PAYLOAD", "")
try:
...
if isinstance(ti, dict):
    for k in ("file_path", "notebook_path"):
        ...
        if isinstance(v, str):
            target = v
            break
if not target or not target.replace("\\", "/").endswith("/reports/test-authoring.md"):
    sys.exit(0)
...
if missing:
    deny("test-authoring.md is missing required produces field(s): " + ", ".join(missing))
sys.exit(0)
PY
